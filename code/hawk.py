...
# This file was *autogenerated* from the file hawk.sage
...
class CyclotomicTower:
    def __init__(CT, top_n):
        ...
        Q(zeta_{2*top_n}).
        """
        CT.top_ln = log(top_n)/log(_sage_const_2 )
        if CT.top_ln != round(CT.top_ln):
            raise ValueError("Cyclotomic degree must be a power of 2")

        CT.top_K = CyclotomicField(_sage_const_2 *top_n)
        CT.Ks = [CyclotomicField(_sage_const_2 **(i+_sage_const_1 )) for i in range(CT.top_ln)] + [CT.top_K]  # noqa
    ...
    def tau(CT, f):
        ...
        """
        n, _, K = CT.level(f)
        return K([f[i] * (-_sage_const_1 )**i for i in range(n)])
    ...
    def norm_down(CT, f):
        ...
        n, ln, _ = CT.level(f)
        ff = f*CT.tau(f)
        return CT.Ks[ln-_sage_const_1 ](list(ff)[::_sage_const_2 ])
    ...
    def partition_round(CT, x):
        ...
        """
        if x < _sage_const_0  and float(_sage_const_2  * x) % _sage_const_1  == _sage_const_0  and float(x) % _sage_const_1  != _sage_const_0 :
            return _sage_const_1  - round(-x)
        else:
            return round(x)
...
class SignatureScheme:
    def __init__(self, n, sigma_kg, sigma_sig, sigma_ver):
        ...
        self.verif_bound = _sage_const_2  * n * (_sage_const_2  * sigma_ver) ** _sage_const_2 
    ...
    def Dsig(self, x):
        ...
        """
        if x % _sage_const_2 :
            return self.D1_sig()[_sage_const_0 ] - _sage_const_1 
        else:
            return self.D0_sig()[_sage_const_0 ]
    ...
    def sym_break(self, h1, s1):
        ...
        """
        e = h1 - _sage_const_2 *s1

        return e != _sage_const_0  and next(filter(lambda x: x != _sage_const_0 , e)) > _sage_const_0 
    ...
    def Sign(self, sk, h):
        ...
        f, g, F, G = sk
        K = self.CT.top_K
        h0 = K(h[_sage_const_0 ])
        h1 = K(h[_sage_const_1 ])
        ...
        s0 = (h0 - e0) / _sage_const_2 
        s1 = (h1 - e1) / _sage_const_2 
    ...
    def Vf(self, pk, h, sig):
        ...
        e0 = K(h0) - _sage_const_2  * s0
        e1 = K(h1) - _sage_const_2  * s1

        v = e0*q00*bar(e0) + e1*q11*bar(e1) + e1*q10*bar(e0) + e0*bar(q10*e1)
        length = v[_sage_const_0 ] <= self.verif_bound
    ...
    def CompressPK(self, pk):
        ...
        """
        (q00, q10, q11) = pk
        q00_ = [q00[i] for i in range(self.n / _sage_const_2 )]
        return (q00_, q10)
    ...
    def DecompressPK(self, pkc):
        ...
        q00 = q00_ + [_sage_const_0 ] + [-q00_[i] for i in range(self.n / _sage_const_2  - _sage_const_1 , _sage_const_0 , -_sage_const_1 )]
        q00 = self.CT.top_K(q00)
        return (q00, q10, (_sage_const_1  + q10*self.CT.bar(q10))/q00)
    ...
    def DecompressSK(self, skc):
        ...
        """
        (f, g, F) = skc
        return (f, g, F, (_sage_const_1  + g*F) / f)
    ...
    def DecompressSig(self, pk, h, s1):
        ...
        (q00, q10, _) = pk
        (h0, h1) = h
        s0 = self.CT.partition_round_poly(K(h0) / _sage_const_2  + (K(h1) / _sage_const_2  - s1) * q10 / q00) # noqa
        return s0, s1


def test_run(n=_sage_const_32 , sigma_kg=_sage_const_1p5 , sigma_sig=_sage_const_1 , sigma_ver=_sage_const_1p1 ):
    ...
    h = ([randint(_sage_const_0 , _sage_const_1 ) for _ in range(n)], [randint(_sage_const_0 , _sage_const_1 ) for _ in range(n)])
